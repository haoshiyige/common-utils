...
const isCorePackage = pkgName => {
  ...
  if (pkgName === 'nin' || pkgName === '@nin/compat') {
    return true
  }

  return pkgName.startsWith('@nin') && packages.includes(pkgName.replace(/^@vue\//, ''))
}
...
async function main() {
  ...
  if (isCanary) {
    ...
    try {
      ...
      const { stdout } = await run('pnpm', ['view', `${pkgName}@~${canaryVersion}`, 'version', '--json'], {
        stdio: 'pipe'
      })
      let versions = JSON.parse(stdout)
      versions = Array.isArray(versions) ? versions : [versions]
      const latestSameDayPatch = /** @type {string} */ (semver.maxSatisfying(versions, `~${canaryVersion}`))
  ...
  if (skipPrompts) {
    step(isCanary ? `Releasing canary version v${targetVersion}...` : `Releasing v${targetVersion}...`)
  } else {
    // @ts-ignore
...
async function getCIResult() {
  try {
    ...
    // const res = await fetch(`https://api.github.com/repos/vuejs/core/actions/runs?head_sha=${sha}` + `&status=success&exclude_pull_requests=true`)
    const res = await fetch(`https://api.github.com/repos/vuejs/core/actions/runs?head_sha=${sha}` + `&status=success&exclude_pull_requests=true`)
